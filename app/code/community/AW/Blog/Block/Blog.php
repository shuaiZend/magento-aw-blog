...
class AW_Blog_Block_Blog extends AW_Blog_Block_Abstract {
	...
	public function getPostsBaseUrl() {
		$baseUrl    = Mage::getStoreConfig('web/secure/base_url');
		$postsRoute = Mage::getStoreConfig('blog/blog/route');

		return "{$baseUrl}{$postsRoute}";
	}

	public function getPostYears() {
		/* @var AW_Blog_Model_Post $model */
		$model = Mage::getModel('blog/post');

		return $model->getPostYears();
	}

	public function getPostsByYear($year = null) {
		if ( is_null($year) ) {
			return [];
		}
		...
		return $model->getPostsByYear($year);
	}
